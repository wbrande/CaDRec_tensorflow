...
## Carl - New class Dataset for implementing tensorFlow
class Dataset(object):
    def __init__(self):
        self.user_num = C.USER_NUMBER
        self.item_num = C.ITEM_NUMBER
        self.directory_path = f'./data/{C.DATASET}/'
    ...
    def parse(self, data):
        """ Parse dataset text files into structured lists. """
        user_traj = [[] for i in range(self.user_num)]
        user_times = [[] for j in range(self.user_num)]
        ...
        for eachline in data:
            ...
            try:
                ...
                print("Error processing User:", uid, len(user_traj))

        return user_traj, user_times

    def read_data(self):
        """ Create user training and validation data """
        user_data = []
        user_valid = []
    ...
    def read_test_data(self):
        test_file = f"{C.DATASET}_test.txt"
        return self.parse(open(self.directory_path + test_file, 'r').readlines())

    def load_adjacent_matrix(self):
        """ Load or Create adjacency matrix for hypergraph convolution """
        ...
        return tf.convert_to_tensor(self.normalize_graph_mat(ui_adj).toarray(), dtype = tf.float32)
    ...
    def normalize_graph_mat(self, adj_mat):
        ...
        ## Carl - Square matrix normalizing
        if shape[0] == shape[1]:
            d_inv = np.power(rowsum, -0.5).flatten()
            d_inv[np.isinf(d_inv)] = 0.
            d_mat_inv = sp.diags(d_inv)
            norm_adj_tmp = d_mat_inv.dot(adj_mat)
            norm_adj_mat = norm_adj_tmp.dot(d_mat_inv)
        ## Carl - Rectangular Matrix normalizing
        else:
            d_inv = np.power(rowsum, -1).flatten()
            d_inv[np.isinf(d_inv)] = 0.
            d_mat_inv = sp.diags(d_inv)
    ...
    def get_in_degree(self):
        """ Compute in-degree for each item in the dataset """
        in_degree = np.zeros(C.ITEM_NUMBER + 1)
        directory_path = f'./data/{C.DATASET}/'
        ...
        if os.path.exists(directory_path + 'in_degree.npy'):
            ...
            all_train_data = open(directory_path + f'{C.DATASET}_train.txt', 'r').readlines()
            all_tune_data = open(directory_path + f'{C.DATASET}_tune.txt', 'r').readlines()
            all_train_data.extend(all_tune_data)
        ...
        return tf.convert_to_tensor(in_degree, dtype = tf.float32)

    def paddingLong2D(self, insts):
        """ Pad the instance to the max seq length in batch. """
        # max_len = 400
        max_len = max(len(inst) for inst in insts)
        batch_seq = np.array([inst[:max_len] + [C.PAD] * (max_len - len(inst)) for inst in insts])
        return tf.convert_to_tensor(batch_seq, dtype = tf.int64)
    ...
    def padding2D(self, insts):
        ...
        max_len = max(len(inst) for inst in insts)
        batch_seq = np.array([inst[:max_len] + [C.PAD] * (max_len - len(inst)) for inst in insts])
        return tf.convert_to_tensor(batch_seq, dtype = tf.float32)

    ## Carl - Not needed for tensorflow
    # def user_fn(self, insts):
    #     """ Collate function, as required by PyTorch. """
    #     (useridx, event_type, event_time, test_label) = list(zip(*insts))
    #     useridx = torch.tensor(useridx, device='cuda:0')
